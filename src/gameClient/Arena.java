...
public class Arena {
	...
	public static void updateEdge(CL_Pokemon pokemon, directed_weighted_graph g) {
		...
		while(itr.hasNext()) {
			...
			Iterator<edge_data> iter = g.getE(next.getKey()).iterator();
			while(iter.hasNext()) {
				edge_data edgeNext = iter.next();
				boolean f = isOnEdge(pokemon.getLocation(), edgeNext,pokemon.getType(), g);
				if(f) {pokemon.set_edge(edgeNext);}
			}
		}
	}
